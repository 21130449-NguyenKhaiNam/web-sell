			...
			<div class="container-xl">
				<div class="row">
							...
							<form class="form" action="SearchFilterOrderAdmin" method="get">
								<div class="search__filter">
									<article class="form__search-block">
										<i class="search__icon fa-solid fa-magnifying-glass"></i> <input
											type="text" name="contentSearch"
											<c:if test="${requestScope.contentSearched != null}">value="${requestScope.contentSearched}"</c:if>
											placeholder="Nhập nội dung tìm kiếm"> <select
											name="searchSelect" class="search__select">
											<option value="orderId"
												<c:if test="${requestScope.searchSelected eq 'orderId'}">selected</c:if>
												class="search__option">Mã đơn hàng</option>
											<option value="customerName"
												<c:if test="${requestScope.searchSelected eq 'customerName'}">selected</c:if>
												class="search__option">Tên khách hàng</option>
										</select>
									</article>
									...
									<div class="group__filter">
										<div class="sections__filter">
											<article class="filler__block">
												<div class="filler__container">
													...
													<div class="interval__filler">
														<div class="filler__date">
															<span class="filler__title">Từ ngày</span> <input
																type="date" name="startDate"
																<c:if test="${requestScope.startDateFiltered != null}">value="${requestScope.startDateFiltered}"</c:if>
																id="date-start">
														</div>
														<div class="filler__date">
															<span class="filler__title">Đến ngày</span> <input
											...
											<article class="filler__block">
												<div class="filler__container">
													...
													<div class="filter__content">
														<c:forEach
															items="${requestScope.listAllDeliveryMethodManage}"
															var="deliveryMethod">
															<label class="filter__label check"> <input
																type="checkbox" name="deliveryMethod"
																<c:if test="${listCheckedDMS != null && listCheckedDMS.contains(String.valueOf(deliveryMethod.id))}">checked</c:if>
																value="${deliveryMethod.id}" class="filter__input"
																hidden="true"> <span>${deliveryMethod.typeShipping}</span>
															</label>
														</c:forEach>
													</div>
												</div>
											...
											<article class="filler__block">
												<div class="filler__container">
													...
													<div class="filter__content">
														<c:forEach
															items="${requestScope.listAllPaymentMethodManage}"
															var="paymentMethod">
															<label class="filter__label check"> <input
																type="checkbox" name="paymentMethod"
																<c:if test="${listCheckedPMS != null && listCheckedPMS.contains(String.valueOf(paymentMethod.id))}">checked</c:if>
																value="${paymentMethod.id}" class="filter__input"
																hidden="true"> <span>${paymentMethod.typePayment}</span>
															</label>
														</c:forEach>
													</div>
												</div>
												...
												<div class="filler__container">
													...
													<div class="filter__content">
														<c:forEach items="${requestScope.listAllOrderStatus}"
															var="orderStatus">
															<label class="filter__label check"> <input
																type="checkbox" name="orderStatus"
																<c:if test="${listCheckedOS != null && listCheckedOS.contains(String.valueOf(orderStatus.id))}">checked</c:if>
																value="${orderStatus.id}" class="filter__input"
																hidden="true"> <span>${orderStatus.typeStatus}</span>
															</label>
														</c:forEach>
													</div>
												</div>
											...
											<article class="filler__block">
												<div class="filler__container">
													...
													<div class="filter__content">
														<c:forEach
															items="${requestScope.listAllTransactionStatus}"
															var="transactionStatus">
															<label class="filter__label check"> <input
																type="checkbox" name="transactionStatus"
																<c:if test="${listCheckedTS != null && listCheckedTS.contains(String.valueOf(transactionStatus.id))}">checked</c:if>
																value="${transactionStatus.id}" class="filter__input"
																hidden="true"> <span>${transactionStatus.typeStatus}</span>
															</label>
														</c:forEach>
													</div>
												</div>
										...
										<div class="button__control">
											<input type="submit" class="apply__filter"
												value="Tìm kiếm và lọc đơn hàng" /> <input type="reset"
												class="reset__filter" value="Hoàn tác tất cả" />
										</div>
									</div>
								</div>
					...
					<div class="col-8">
						...
						<%--                        <ul class="paging"></ul>--%>
						<%--                    </div>--%>
						<form id="process__order--form" action="ProcessOrderAdmin"
							method="post">
							...
							<div class="table__wrapper">
								<table class="table table__order">
									...
									<tbody>
										...
										<%--                            </c:if>--%>
										<c:forEach items="${requestScope.listOrdersPerPage}"
											var="order">
											<c:set var="notAllow"
												value="${order.orderStatusId == 5 ? 'not__allow' : ''}" />
											<tr class="table__row ${notAllow}">
												<td class="table__data"><label
													class="filter__label check"> <input type="checkbox"
														name="multipleOrderId" value="${order.id}"
														class="filter__input" hidden="true">
												</label></td>
												<td class="table__data">
													<button type="button" name="seeDetailId"
														data-action="seeDetail" value="${order.id}"
														class="table__cell see__detail">
														<i class="fa-solid fa-eye"></i>
													</button>
												</td>
												<td class="table__data">
													<button type="button" name="showDialogUpdateId"
														value="${order.id}" data-action="showDialogUpdate"
														class="table__cell show__update">
												...
												<td class="table__data"><p class="table__cell">${adminOrderServices.getDeliveryMethodManageById(order.deliveryMethodId).typeShipping}</p>
												</td>
												<td class="table__data"><p class="table__cell">${adminOrderServices.getPaymentMethodMangeById(order.paymentMethodId).typePayment}</p>
												</td>
							...
							<!--Paging-->
							<c:set value="${requestScope.page}" var="page" />
							<c:set value="${requestScope.queryStringFilter}"
							...
							<div class="pagination">
								<c:if test="${page > 1}">
									...
									<a href="${prevURLPage.replaceAll('&$', '')}"
										class="previous__page"><i class="fa-solid fa-chevron-left"></i></a>
								</c:if>
								<c:forEach begin="${1}" end="${requestScope.totalPage}" var="i">
									<c:url var="trURLPage"
										value="${requestScope.servletProcess}?${queryStringFilter != null ? queryStringFilter : ''}">
								...
								<c:if test="${page < requestScope.totalPage}">
									...
									<a href="${nextURLPage.replaceAll('&$', '')}"
										class="next__page"><i class="fa-solid fa-chevron-right"></i></a>
								</c:if>
							</div>
							...
							<div id="dialog__order" class="modal">
								<article class="modal__content modal__order"></article>
