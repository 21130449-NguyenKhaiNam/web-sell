				...
				<div class="row ">
					...
					<div class="col-9">
							...
							<div class="product__list">
								<c:forEach var="item" items="${list}">
									<div class="product__item">
										...
										<c:if
											test="${!productFactory.getListImagesByProductId(item.id).isEmpty()}">
											<c:set var="image"
												value="${productFactory.getListImagesByProductId(item.id).get(0).nameImage}" />
										</c:if>
										<img
											src="${pageContext.servletContext.contextPath}/assets/img/product_img/${image}"
										...
										<div class="product__info">
											...
											<span class="product__price"> <fmt:formatNumber
													value="${item.originalPrice}" type="currency"
													currencyCode="VND" var="originalPrice" /> <c:choose>
													<c:when test="${item.salePrice != null}">
														<strong class="product__price--sale"> <fmt:formatNumber
																value="${item.salePrice}" type="currency"
																currencyCode="VND" var="salePrice" /> ${salePrice}
														</strong>
														<strong class="product__price--original">
															${originalPrice} </strong>
													</c:when>
													<c:otherwise>
						...
						<c:if test="${empty list}">
							<p class="product__list--empty">Không có sản phẩm nào ứng với
								bộ lọc</p>
						</c:if>
